...
/**  * Initializes the left and right IR sensors.  *  * The run method adds the appropriate event to the blocking queue when one of the IR sensors is triggered.  *  *  * @author J.P. Ertola and Paul Thurston  *  */ public class IRSensor extends Thread {
...
public IRSensor(BlockingQueue buf, float threshold, int priority, int
period){
mBuf = buf; mThreshold = threshold; mPeriod = period;
try{ rangeFinderLeft = new SharpGP2D12(IntelliBrain.getAnalogInput(1), null);
rangeFinderRight = new
SharpGP2D12(IntelliBrain.getAnalogInput(2), null);
}catch(Exception e){
		System.out.print("t1");
		} setPriority(priority); setDaemon(true); start();
	}
	public void run()
		{ try{
			while(true){
